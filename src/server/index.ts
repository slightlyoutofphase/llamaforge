...
async function main() {
  ...
  // Populate metadata and generate any missing default presets synchronously at startup.
  try {
    ...
    logInfo("Synchronous model metadata population complete.");
  } catch (e) {
  ...
  if (settings.autoloadLastModel && settings.lastLoadConfig && settings.llamaServerPath) {
    try {
      logInfo(`Autoloading last model: ${settings.lastLoadConfig.modelPath}`);
      await loadModel(
      ...
      logInfo("Autoload completed successfully.");
    } catch (err) {
      logError(
  ...
  const cleanup = () => {
    ...
    logInfo("Shutting down LlamaForge Server...");
    try {
      const proc = getProc();
      if (proc) {
